...
class Discretizer:
    def __init__(self, timestep=0.8, store_masks=True, impute_strategy='zero', start_time='zero',
                 config_path=os.path.join(os.path.dirname(__file__), 'resources/discretizer_config_2.json')):
    # def __init__(self, timestep=0.8, store_masks=True, impute_strategy='zero', start_time='zero',
    #              config_path=os.path.join(os.path.dirname(__file__), 'resources/discretizer_config_2_301.json')):
    ...
    def transform(self, X, header=None, end=None):
        ...
        max_hours = 50.0
        # if ((max(ts) - first_time) > max_hours):
        #     print("time quantum > max_hours !!!")
        ...
        def write(data, bin_id, channel, value, begin_pos):
            ...
            if self._is_categorical_channel[channel]:
                # 粪便颜色有的数据没有映射，”红棕色“映射成”Maroon“；其他的映射成others，以防后面再出现新标签。
                if value == "红棕色":
                    value = "Maroon"
                if value not in self._possible_values[channel]:
                    value = "Other/Remarks"
        ...
        for row in X:
            ...
            for j in range(1, len(row)):
                if row[j] == "":
                    continue
                # 301数据清除字符串数据，"大便颜色"除外
                channel = header[j]
                if channel != "Stool Color":
                    if not isFloat(row[j]):
                        continue
                channel_id = self._channel_to_id[channel]
