...
def main():
    ...
    # 改名后检索病人号需要改
    beginIndex = 4
    train_lines = [x for x in lines if x[beginIndex:x.find("_", beginIndex)] not in val_patients]
    val_lines = [x for x in lines if x[beginIndex:x.find("_", beginIndex)] in val_patients]
