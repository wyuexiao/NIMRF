...
from keras.layers import Input, Dense, LSTM, Masking, Dropout, LeakyReLU
...
class Network(Model):
    ...
    def __init__(self, dim, batch_norm, dropout, rec_dropout, task,
        ...
        mX = Masking()(X)
        mX = Dense(1)(mX)
        mX = LeakyReLU(alpha=0.1)(mX)
        # mX = keras.layers.Conv1D(filters = 1, kernel_size = 1, strides=1, use_bias=False, name='conv1/conv')(X)
        # mX = keras.layers.GlobalAveragePooling1D()(mX)
        ...
        if target_repl:
            ...
            # y = Dense(num_classes, activation=final_activation)(x)
