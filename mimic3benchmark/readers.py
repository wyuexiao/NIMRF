...
class InHospitalMortalityReader(Reader):
    ...
    def _read_timeseries(self, ts_filename):
        ...
        # 医院数据读excel
        # raw_data = pd.read_excel(os.path.join(self._dataset_dir, ts_filename), header=None)
        # example_info_list = raw_data.values.tolist()
        # ret = []
        # header = example_info_list[0]
        # assert header[0] == "Hours"
        # for i in range(1, len(example_info_list)):
        #     for j in range(len(example_info_list[i])):
        #         example_info_list[i][j] = str(example_info_list[i][j])
        #         if example_info_list[i][j] == "nan":
        #             example_info_list[i][j] = ''
        #     ret.append(np.array(example_info_list[i]))
        # return (np.stack(ret), header)
